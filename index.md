...
- instead of
  ...
  use
  [`ggdensity::geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md);
- instead of
  [`ggplot2::geom_density_2d()`](https://ggplot2.tidyverse.org/reference/geom_density_2d.html),
  use
  [`ggdensity::geom_hdr_lines()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md).

Also included are the functions
[`geom_hdr_fun()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_fun.md)
and
[`geom_hdr_lines_fun()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_fun.md)
for plotting HDRs of user-specified bivariate probability density
functions.
...
variables in **ggplot2** is to use
[`ggplot2::geom_density_2d()`](https://ggplot2.tidyverse.org/reference/geom_density_2d.html)
or
...
![](reference/figures/README-ex0-1.png)

While it’s a nice looking plot, it isn’t immediately clear how we should
understand it. That’s because
[`geom_density_2d_filled()`](https://ggplot2.tidyverse.org/reference/geom_density_2d.html)
generates its contours as equidistant level sets of the estimated
bivariate density, i.e. taking horizontal slices of the 3d surface at
equally-spaced heights, and projecting the intersections down into the
plane. So you get a general feel of where the density is high, but not
much else. To interpret a contour, you would need to multiply its height
by the area it bounds, which of course is very challenging to do by just
looking at it.

[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
tries to get around this problem by presenting you with regions of the
estimated distribution that are immediately interpretable:

``` r
p + geom_hdr()
```

![](reference/figures/README-ex1-1.png)
...
be changed with the `probs` argument to
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md).
Notice that your take-away from the plot made with
[`geom_density_2d_filled()`](https://ggplot2.tidyverse.org/reference/geom_density_2d.html)
is subtlely yet significantly different than that of the plot made by
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md).
...
For example, because
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
maps probability to the `alpha` aesthetic, the `fill` and `color`
aesthetics are available for mapping to variables. You can use them to
visualize subpopulations in your data. For example, in the `penguins`
data from
...
![](reference/figures/README-ex_penguins-1.png)
...
![](reference/figures/README-ex_penguins_lines-1.png)
...
![](reference/figures/README-ex_penguins_facet-1.png)

The main point here is that you should really think of
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
and
[`geom_hdr_lines()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
...
and so on, and you can expect all of the rest of the **ggplot2** stuff
to just work.

## A deeper cut illustrating **ggplot2** integration

The underlying stat used by
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
creates the computed variable `probs` that can be mapped in the standard
way you map computed variables in **ggplot2**, with
...
and
[`geom_hdr_lines()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
map `probs` to the `alpha` aesthetic by default. But you can override it
like this, just be sure to override the `alpha` aesthetic by setting
`alpha = 1`.
...
![](reference/figures/README-ex_after_stat-1.png)
...
![](reference/figures/README-ex_after_stat-2.png)
...
To that end, you can pass a `method` argument into
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
and
[`geom_hdr_lines()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
that allows you to specify various nonparametric and parametric ways to
estimate the underlying bivariate distribution, and we have plans for
even more. Each of the estimators below offers advantages in certain
contexts. For example, histogram estimators result in HDRs that obey
constrained supports. Normal estimators can be helpful in providing
simplified visuals that give the viewer a sense of where the
distributions are, potentially at the expense of over-simplifying and
removing important features of how the variables (co-)vary.

![](reference/figures/README-ex_methods-1.png)
...
governing the density estimation procedure. For example,
...
those circumstances, you can use
[`geom_hdr_fun()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_fun.md)
and
[`geom_hdr_lines_fun()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_fun.md)
to make the analogous plots. These functions behave similarly to
[`geom_function()`](https://ggplot2.tidyverse.org/reference/geom_function.html)
from [**ggplot2**](https://github.com/tidyverse/ggplot2), accepting the
...
![](reference/figures/README-ex_hdr_fun_1-1.png)
...
[`geom_hdr_fun()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_fun.md)
is that it allows you to plot parametric densities that you estimate
outside the **ggdensity** framework. The basic idea is that you fit your
...
the density formula to obtain a function to plug into
[`geom_hdr_fun()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_fun.md).
...
![](reference/figures/README-ex_hdr_fun_2-1.png)
...
![](reference/figures/README-ex_geom_hdr_points-1.png)
...
![](reference/figures/README-ex_geom_hdr_rug_1-1.png)
...
![](reference/figures/README-ex_geom_hdr_rug_2-1.png)

Like
[`geom_rug()`](https://ggplot2.tidyverse.org/reference/geom_rug.html),
these can be placed on different sides of the object:
...
![](reference/figures/README-ex_geom_hdr_rug_3-1.png)
...
For details on the objects returned by these functions, see
[`?get_hdr`](https://jamesotto852.github.io/ggdensity/reference/get_hdr.md)
and
[`?get_hdr_1d`](https://jamesotto852.github.io/ggdensity/reference/get_hdr_1d.md).
...
[`geom_hdr()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr.md)
and related functions were written with the intent of playing nicely
with **ggplot2**, so that what the typical **ggplot2** user would expect
from the rest of the **ggplot2** ecosystem would work in the same way
with **ggdensity**.
...
for example: computed aesthetics don’t extend past the range of the
data.
...
e.g. [`MASS::kde2d()`](https://rdrr.io/pkg/MASS/man/kde2d.html), and
most parametric density estimation, too. In other words: the density
estimate itself is only known at points on a grid over the `x`-`y`
aesthetic space. As a consequence, if that range is too small, it’s
possible that a probabilistically non-trivial proportion of the density
is excluded from the computations, biasing the resulting HDRs.
...
aesthetic limits and use
[`coord_cartesian()`](https://ggplot2.tidyverse.org/reference/coord_cartesian.html)
to zoom in as needed. Here’s an example using the previously created
graphic. The limits given to
[`coord_cartesian()`](https://ggplot2.tidyverse.org/reference/coord_cartesian.html)
and the call to
[`scale_y_continuous()`](https://ggplot2.tidyverse.org/reference/scale_continuous.html)
is simply an effort to make the third plot comparable to the first.
...
![](reference/figures/README-ex-expand-lims-1.png)
...
![](reference/figures/README-hdrcde-1.png)
...
same as
[`ggdensity::geom_hdr_rug()`](https://jamesotto852.github.io/ggdensity/reference/geom_hdr_rug.md);
it implements a kind of `ggdensity::geom_hdr_pointdensity()` via a
function `gghdr::hdr_bin()` plus the color aesthetic to
[`geom_point()`](https://ggplot2.tidyverse.org/reference/geom_point.html);
and it provides a boxplot alternative `gghdr::geom_hdr_boxplot()`. To
the extent the similarities between **ggdensity** and
**hdrcde**/**gghdr** exist (and they obviously do), they are an example
of [convergent
